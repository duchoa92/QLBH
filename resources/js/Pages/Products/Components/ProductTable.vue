...
<template>
    <div class="bg-white rounded-xl shadow border overflow-hidden">
        <div class="overflow-x-auto">
            <table class="min-w-[900px] w-full text-sm border border-gray-200 rounded-lg overflow-hidden">
                <thead >
                    ...
                    >
                        ...
                        <td class="border-r p-2">
                            <div class="flex items-center gap-2">
                                <img
                                    :src="p.image_url || `/storage/${p.image}` || '/no-image.png'"
                        ...
                        <td class="text-center border-r p-2">{{ p.sold_count }}</td>

                        <td class="text-center p-2 text-gray-400">
                            <button @click.stop="$emit('print', p.id)" 
                                    class="mx-1 px-2 py-2 bg-blue-500 text-white rounded text-sm"
                            >
                               <Printer class="w-4 h-4" />
                            </button>
                            <button @click.stop="$emit('delete', p.id)" 
                                    class="mx-1 px-2 py-2 bg-red-500 text-white rounded text-sm"
                            >
    ...
    <div class="flex items-center justify-between px-4 py-3 border-t bg-white text-sm">
        ...
        <div class="text-gray-500 w-1/3">
            Trang {{ products.from }} - {{ products.to }} / {{ products.total }}
        </div>
        ...
        <div class="flex justify-end w-1/3">
            <select
                ...
                class="border rounded px-3 py-1 text-sm w-auto"
            >
